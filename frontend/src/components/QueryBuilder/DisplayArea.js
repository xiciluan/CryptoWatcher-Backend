...
import { Accordion, Button, Container, Dimmer, Loader, Table, Header, Icon } from 'semantic-ui-react'
...
export default function DisplayArea({ formData }) {
  ...
  if (blockByHashLoading || blockByHeightLoading || blocksLoading || walletIncomeLoading) {
    ...
    let errorType = 0
    if (err0 && err0.networkError.name === 'TypeError') {
      errorType = 1
    }
    if (err1 && err1.networkError.name === 'TypeError') {
      errorType = 1
    }
    if (err2 && err2.networkError.name === 'TypeError') {
      errorType = 1
    }
    if (err3 && err3.networkError.name === 'TypeError') {
      errorType = 1
    }
    if (err4 && err4.networkError.name === 'TypeError') {
      errorType = 1
    }
      ...
      <Container text textAlign="center">
        {
          errorType === 0 ? (
            <>
              <Header as="h4">Invalid Input! <Icon name='ban' /></Header>
            </>
          ) : (
              <>
                <Header as="h4">GraphQL backend server is currently paused<Icon name='ban' /></Header>
                <Header as="h4">Please come back later</Header>
